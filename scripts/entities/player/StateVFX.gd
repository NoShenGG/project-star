...
@export var state_overlay : BaseMaterial3D
@export_range(0.01,2, 0.01) var flash_duration : float = 0.3
@export var tween : bool = true
var _overlay_tween : Tween
@onready var original_a : float = state_overlay.albedo_color.a
var flash_count : int = 0

func _enter_tree() -> void:
	(get_parent() as State).entered.connect(state_entered)
	...
	state_overlay = state_overlay.duplicate()

func _exit_tree() -> void:
	(get_parent() as State).entered.disconnect(state_entered)
...
func state_entered():
	...
	if (vfx): vfx.restart_particles()

func flash(duration : float, intensity : float):
	flash_count += 1
	var flash_size : int = flash_count
	for mesh in _meshes:
		mesh.material_overlay = state_overlay
	
	state_overlay.albedo_color.a = original_a
	if (tween):
		if (_overlay_tween): _overlay_tween.stop()
		_overlay_tween = create_tween()
		_overlay_tween.set_trans(_overlay_tween.TRANS_EXPO)
		var new_albedo = state_overlay.albedo_color
		new_albedo.a = 0
		_overlay_tween.tween_property(state_overlay, "albedo_color", new_albedo, flash_duration)
	...
	state_overlay.albedo_color.a = 0
	flash_count -= 1
